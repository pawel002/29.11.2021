...
def king_better(T, pos, end_pos, is_kings_turn):
    if pos == end_pos:
        T[pos[0]][pos[1]] *= -1
        return True
    ...
    for D in directions:
        ...
        if 0 <= next_pos[0] <= 7 and 0 <= next_pos[1] <= 7 and T[next_pos[0]][next_pos[1]] > 0:
            ...
            if var:
                T[pos[0]][pos[1]] *= -1
                if king_better(T, end_pos, next_pos, not is_kings_turn):
                    return True
                T[pos[0]][pos[1]] = abs(T[pos[0]][pos[1]])
    return False


def quadCorners(T, start_pos):
    corners = [[0, 0], [0, 7], [7, 0], [7, 7]]
    for corner in corners:
        if king_better(T, start_pos, corner, True):
            for row in T:
                for ele in row:
                    if ele < 0:
